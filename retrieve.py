...
info_ids_selector = "city, acceptanceRate, acceptanceRate, sat_average, act_cumulative_midpoint, gpa_bottom_ten_percent, admissions_website, undergraduate_size".replace(',', '%2C')
...
def get_college_info(college_name):
    ...
    try:
        ...
        return 1
    url = f'https://api.collegeai.com/v1/api/college/info?api_key={api_key}&college_unit_ids='+str(college_id)+'&info_ids='+info_ids_available
...
x = get_college_info("harvard")
f.writerow(list(x.keys()))
